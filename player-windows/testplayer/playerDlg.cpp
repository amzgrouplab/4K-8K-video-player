...
void CplayerDlg::PlayerReset(PLAYER_INIT_PARAMS *params)
{
    ...
    if (m_bPlayerMode == true) {
        m_ffPlayer = player_open(m_filePath, GetSafeHwnd(), params);
    }
    else {

        CFileDialog dlg(TRUE);
        char  front_path[MAX_PATH];
        TCHAR       str[MAX_PATH];
        ...
        WideCharToMultiByte(CP_UTF8, 0, str, -1, front_path, MAX_PATH, NULL, NULL);

        // set window title
        SetWindowText(TEXT("testplayer - loading"));

        // player open file
        char ext[MAX_PATH]; _splitpath(front_path, NULL, NULL, NULL, ext);
        m_ffPlayer = playBackwordVideo(m_filePath, front_path, GetSafeHwnd(), params);
    }
}
...
void CplayerDlg::PlayerOpenFile(TCHAR *file)
{
    ...
    if (!file) {
        if (dlg.DoModal() == IDOK) {
            _tcscpy(str, dlg.GetPathName());
            WideCharToMultiByte(CP_UTF8, 0, str, -1, m_filePath, MAX_PATH, NULL, NULL);

            // set window title
            SetWindowText(TEXT("testplayer - loading"));

            // player open file
            char ext[MAX_PATH]; _splitpath(m_filePath, NULL, NULL, NULL, ext);
...
BEGIN_MESSAGE_MAP(CplayerDlg, CDialog)
    ...
    ON_COMMAND(ID_VOLUME_UP, &CplayerDlg::OnVolumeUp)
    ON_COMMAND(ID_VOLUME_DOWN, &CplayerDlg::OnVolumeDown)
    ON_COMMAND(ID_SINGLE_PLAYER, &CplayerDlg::OnSinglePlayer)
    ON_COMMAND(ID_REAR_PLAYER, &CplayerDlg::OnRearPlayer)
    ON_COMMAND(ID_STOP, &CplayerDlg::OnStop)
    ON_COMMAND(ID_GET_FPS, &CplayerDlg::OnGetFps)
END_MESSAGE_MAP()
...
BOOL CplayerDlg::OnInitDialog()
{
    ...
    LOGFONT logfont = {0};
    _tcscpy_s(logfont.lfFaceName, _countof(logfont.lfFaceName), TEXT("����"));
    logfont.lfHeight = 32;
...
void CplayerDlg::OnTimer(UINT_PTR nIDEvent)
{
    ...
    case TIMER_ID_DISP_DEFINITIONVAL: {
            ...
            _stprintf(m_strTxt, TEXT("������ %3.1f"), val);
            player_textout(m_ffPlayer, m_hFont, 20, 20, 360, 50, RGB(0, 255, 0), 200, m_strTxt);
        }
        break;
...
BOOL CplayerDlg::PreTranslateMessage(MSG *pMsg) 
{
    ...
    if (pMsg->message == MSG_FANPLAYER) {
        ...
        case MSG_AV1_CODEC:
            OnStop();
            AfxMessageBox(_T("This video encoded in AV01 format which is not supported"), MB_OK);
            break;
        case MSG_NOT_4K:
            OnStop();
            AfxMessageBox(_T("This video is not 4K video. Check this video again"), MB_OK);
            break;
        case MSG_GET_FPS:
            float frame_rate;
            ...
            if (frame_rate > 30) {
                ...
                CString message;
                message.Format(_T("The video frame rate is %s \n The current video player only works with videos that have a frame rate of less than 30 fps."), frame_rate_str);
...
void CplayerDlg::OnVolumeUp()
{
    //increase volume
    int volume;
    player_getparam(m_ffPlayer, PARAM_AUDIO_VOLUME, &volume);
    volume += 20; if (volume > 73) volume = 73;
    player_setparam(m_ffPlayer, PARAM_AUDIO_VOLUME, &volume);

    _stprintf(m_strTxt, TEXT("volume value: %d"), (int)((volume+182)*100) / 255);
    PlayerShowText(2000);
}


void CplayerDlg::OnVolumeDown()
{
    //decrease volume
    int volume;
    player_getparam(m_ffPlayer, PARAM_AUDIO_VOLUME, &volume);
    volume -= 20; if (volume < -182) volume = -182;
    player_setparam(m_ffPlayer, PARAM_AUDIO_VOLUME, &volume);
...
void CplayerDlg::OnSinglePlayer()
{
    //single video play
    m_bPlayerMode = true;
    m_nPlayerStatus = 0;
...
void CplayerDlg::OnRearPlayer()
{
    //rear video play with front video audio
    m_bPlayerMode = false;
    m_nPlayerStatus = 0;
    PlayerOpenFile(NULL);
