...
static void vfilter_graph_input(PLAYER *player, AVFrame *frame)
{
    if (player->vfilter_graph) {
        ...
        if (ret != 0) av_log(NULL, AV_LOG_WARNING, "av_buffersrc_add_frame_flags failed !\n");
    }
}
...
static int player_prepare_or_free(PLAYER *player, int prepare)
{
    ...
    if (player->vcodec_context != NULL) {
        if (player->vcodec_context->codec_id == AV_CODEC_ID_AV1) {
            return -2;
        }
    }
...
static int handle_fseek_or_reconnect(PLAYER *player)
{
    ...
    if (!player->avformat_context || (player->status & PS_RECONNECT)) {
        if (ret == 0) player_send_message(player->cmnvars.winmsg, MSG_STREAM_DISCONNECT, player);
        ret = player_prepare_or_free(player, 1);
        //check codec
        if(ret == -2) return ret;
        if (ret == 0) player_send_message(player->cmnvars.winmsg, MSG_STREAM_CONNECTED , player);
    } else {
...
static void* av_demux_thread_proc(void *param)
{
    ...
    float     frame_rate;
    while (!(player->status & PS_CLOSE)) {
        int ret = handle_fseek_or_reconnect(player);//-2 -> AV1 codec
        if (ret == -2) {
            player_send_message(player->cmnvars.winmsg, MSG_AV1_CODEC, 0);
            break;
        }
        if ( ret != 0) {
        ...
        if (frame_rate > 30) {
            player_send_message(player->cmnvars.winmsg, MSG_GET_FPS, 0);
            break;
        }
...
{
    //sa-added
    MAINPLAYER* mainplayer = (MAINPLAYER*)calloc(1, sizeof(MAINPLAYER));
    mainplayer->front_player = (PLAYER*)calloc(1, sizeof(PLAYER));

    //PLAYER *player = (PLAYER*)calloc(1, sizeof(PLAYER));
    if (!mainplayer->front_player) return NULL;
    ...
    pthread_mutex_init(&mainplayer->front_player->lock, NULL); // init lock
    mainplayer->front_player->status = (PS_A_PAUSE | PS_V_PAUSE | PS_R_PAUSE); // make sure player paused

    // create packet queue
    mainplayer->front_player->pktqueue = pktqueue_create(0, &mainplayer->front_player->cmnvars);
    if (!mainplayer->front_player->pktqueue) {
        av_log(NULL, AV_LOG_ERROR, "failed to create packet queue !\n");
        goto error_handler;
    }

    // for player init params
    if (params) memcpy(&mainplayer->front_player->init_params, params, sizeof(PLAYER_INIT_PARAMS));
    mainplayer->front_player->cmnvars.init_params = &mainplayer->front_player->init_params;

    //++ for player_prepare
    strcpy(mainplayer->front_player->url, file);

#ifdef WIN32
    mainplayer->front_player->cmnvars.winmsg = win;
#endif
    mainplayer->front_player->vfrate.num = 20, mainplayer->front_player->vfrate.den = 1;

    pthread_create(&mainplayer->front_player->avdemux_thread, NULL, av_demux_thread_proc, mainplayer->front_player);
    pthread_create(&mainplayer->front_player->adecode_thread, NULL, audio_decode_thread_proc, mainplayer->front_player);
    pthread_create(&mainplayer->front_player->vdecode_thread, NULL, video_decode_thread_proc, mainplayer->front_player);
    return mainplayer; // return

error_handler:
    player_close(mainplayer);
    return NULL;
}
void* playBackwordVideo(char* rearvideo_path, char* frontvideo_path, void* win, PLAYER_INIT_PARAMS* params)
{
    ...
    MAINPLAYER* mainplayer = (MAINPLAYER*)calloc(1, sizeof(MAINPLAYER));
    mainplayer->front_player = (PLAYER*)calloc(1, sizeof(PLAYER));
    mainplayer->rear_player = (PLAYER*)calloc(1, sizeof(PLAYER));
    if (!mainplayer->rear_player) return NULL;
    if (!mainplayer->front_player) return NULL;
    // av register all
    av_register_all();
    avdevice_register_all();
    ...
    pthread_mutex_init(&mainplayer->rear_player->lock, NULL); // init lock
    mainplayer->rear_player->status = (PS_A_PAUSE | PS_V_PAUSE | PS_R_PAUSE); // make sure player paused

    // create packet queue
    mainplayer->rear_player->pktqueue = pktqueue_create(0, &mainplayer->rear_player->cmnvars);
    if (!mainplayer->rear_player->pktqueue) {
        av_log(NULL, AV_LOG_ERROR, "failed to create packet queue !\n");
        goto error_handler;
    }

    // for player init params
    if (params) memcpy(&mainplayer->rear_player->init_params, params, sizeof(PLAYER_INIT_PARAMS));
    mainplayer->rear_player->cmnvars.init_params = &mainplayer->rear_player->init_params;

    //++ for player_prepare
    strcpy(mainplayer->rear_player->url, rearvideo_path);
    ...
    //++ for player_prepare
    strcpy(mainplayer->front_player->url, frontvideo_path);


#ifdef WIN32
    mainplayer->rear_player->cmnvars.winmsg = win;
#endif
    mainplayer->rear_player->vfrate.num = 20, mainplayer->rear_player->vfrate.den = 1;
#ifdef WIN32
    mainplayer->front_player->cmnvars.winmsg = win;
#endif
    mainplayer->front_player->vfrate.num = 20, mainplayer->front_player->vfrate.den = 1;

    mainplayer->rear_player->init_params.audio_stream_cur = -1;
    mainplayer->front_player->init_params.video_stream_cur= -1;

    pthread_create(&mainplayer->rear_player->avdemux_thread, NULL, av_demux_thread_proc, mainplayer->rear_player);
    pthread_create(&mainplayer->front_player->avdemux_thread, NULL, av_demux_thread_proc, mainplayer->front_player);

    pthread_create(&mainplayer->front_player->adecode_thread, NULL, audio_decode_thread_proc, mainplayer->front_player);
    pthread_create(&mainplayer->rear_player->adecode_thread, NULL, audio_decode_thread_proc, mainplayer->rear_player);
    pthread_create(&mainplayer->front_player->vdecode_thread, NULL, video_decode_thread_proc, mainplayer->front_player);
    pthread_create(&mainplayer->rear_player->vdecode_thread, NULL, video_decode_thread_proc, mainplayer->rear_player);
    return mainplayer; // return

error_handler:
    player_close(mainplayer->rear_player);
    player_close(mainplayer->front_player);
    return NULL;
}
...
void player_close(void *hplayer)
{
    ...
    //PLAYER* player = mainplayer->rear_player;
    PLAYER* player;
    if (mainplayer->rear_player) {
        PLAYER * rear_player = mainplayer->rear_player;
        rear_player->read_timeout = 0; // set read_timeout to 0
        pthread_mutex_lock(&rear_player->lock);
        rear_player->status |= PS_CLOSE;
        pthread_mutex_unlock(&rear_player->lock);
        render_pause(rear_player->render, 2);
        if (rear_player->adecode_thread) pthread_join(rear_player->adecode_thread, NULL); // wait audio decoding thread exit
        if (rear_player->vdecode_thread) pthread_join(rear_player->vdecode_thread, NULL); // wait video decoding thread exit
        if (rear_player->avdemux_thread) pthread_join(rear_player->avdemux_thread, NULL); // wait avdemux thread exit
        pthread_mutex_destroy(&rear_player->lock);

#ifdef ENABLE_AVKCP_SUPPORT
        if (rear_player->avkcpd) avkcpdemuxer_exit(rear_player->avkcpd);
#endif
#ifdef ENABLE_AVKCP_SUPPORT
        if (rear_player->ffrdpd) ffrdpdemuxer_exit(rear_player->ffrdpd);
#endif

        player_prepare_or_free(rear_player, 0);  // free resources
        recorder_free(rear_player->recorder); // destroy recorder
        datarate_destroy(rear_player->datarate); // destroy data rate
        pktqueue_destroy(rear_player->pktqueue); // destroy packet queue

#ifdef ANDROID
        JniReleaseWinObj(rear_player->cmnvars.winmsg);
#endif
    ...
    player = mainplayer->front_player;
    ...
    player_prepare_or_free(player, 0);  // free resources
    recorder_free   (player->recorder); // destroy recorder
...
void player_play(void *hplayer)
{
    ...
    //PLAYER *player = (PLAYER*)hplayer;
    //if (!front_player || !front_player->avformat_context) return;
    if (front_player && front_player->avformat_context) {
        pthread_mutex_lock(&front_player->lock);
        front_player->status &= PS_CLOSE;
        pthread_mutex_unlock(&front_player->lock);
        render_pause(front_player->render, 0);
        datarate_reset(front_player->datarate);
    }
}
...
void player_setrect(void *hplayer, int type, int x, int y, int w, int h)
{
    ...
    MAINPLAYER* mainplayer = (MAINPLAYER*)hplayer;
    if (mainplayer->rear_player) {
        render_setrect(mainplayer->rear_player->render, type, x, y, w, h);
    }
    else {
        render_setrect(mainplayer->front_player->render, type, x, y, w, h);
    }
...
void player_seek(void *hplayer, int64_t ms, int type)
{
    ...
    PLAYER* front_player;
    if (mainplayer->rear_player) {
        rear_player = mainplayer->rear_player;
        if (rear_player->status & (PS_F_SEEK | rear_player->seek_req)) {
        ...
        switch (type) {
            ...
            case SEEK_STEP_BACKWARD:
                ...
                break;
            default:
                rear_player->seek_dest = rear_player->cmnvars.start_time + ms;
                rear_player->seek_pos = (rear_player->cmnvars.start_time + ms) * AV_TIME_BASE / 1000;
                rear_player->seek_diff = 1000; // Play position event (in 1 second, callback) provided
                rear_player->seek_sidx = -1;
                break;
        }
        ...
        // set PS_F_SEEK flag
        pthread_mutex_lock(&rear_player->lock);
        rear_player->status |= PS_F_SEEK;
        pthread_mutex_unlock(&rear_player->lock);
    }


    front_player = mainplayer->front_player;
    if (front_player->status & (PS_F_SEEK | front_player->seek_req)) {
        av_log(NULL, AV_LOG_WARNING, "seek busy !\n");
        return;
    }
    switch (type) {
...
void player_setparam(void *hplayer, int id, void *param)
{
    ...
    if (mainplayer->rear_player) {
        ...
        case PARAM_FFRDP_SENDDATA: {
            ...
            ffrdpdemuxer_senddata(mainplayer->rear_player->ffrdpd, data->data, data->size);
        }
                                 break;
#endif
        default: render_setparam(mainplayer->rear_player->render, id, param); break;
        }
    }

    PLAYER* player = mainplayer->front_player;
...
void player_getparam(void *hplayer, int id, void *param)
{
    ...
    case PARAM_PLAYER_INIT_PARAMS:
        ...
        break;
    case PARAM_FRAMERATE_VALUE:
        if (!player->vcodec_context) *(float*)param = 0;
        
        else {
            float frame_rate = (float)player->vfrate.num / player->vfrate.den;
            *(float*)param = frame_rate;
        }
        break;
